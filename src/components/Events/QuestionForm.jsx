...
const QuestionForm = ({ eventId, question, onClose, eventType }) => {
  const { addQuestion, updateQuestion } = useEvents();

  const normalizedChoices = (() => {
    const incomingChoices = Array.isArray(question?.choices)
      ? question.choices
      : [];

    if (eventType === "leaderboard_quiz") {
      return [
        ...incomingChoices.slice(0, 4),
        ...Array(Math.max(0, 4 - incomingChoices.length)).fill(""),
      ];
    }

    return incomingChoices.length > 0 ? incomingChoices : ["", "", "", ""];
  })();
      ...
      {eventType === "leaderboard_quiz" && (
        <div>
          ...
          <textarea
            ...
            className={`mt-1 input ${errors.explanation ? "border-red-500" : ""}`}
          />
