...
const LessonDetails = ({ lesson, onClose }) => {
  ...
  const vocabularies = getVocabularies(lesson.id);
  ...
  const confirmDelete = () => {
    if (deleteType === 'vocabulary') {
      deleteVocabulary(lesson.id, itemToDelete.id);
    } else if (deleteType === 'question') {
      deleteQuestion(lesson.id, itemToDelete.id);
    }
  };
  ...
  const vocabularyColumns = [
    ...
    { field: 'translatedWord', header: 'Translated Word', sortable: true },
    { 
      field: 'image', 
      header: 'Image', 
      sortable: false,
  ...
  const questionColumns = [
    ...
    { 
      ...
      sortable: true,
      render: (row) => (
        <div className="truncate max-w-xs">{row.correctAnswer}</div>
      )
    },
    { 
      field: 'choices', 
    ...
    <div className="space-y-6">
      <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 rounded-lg">
        ...
        <div className="mt-4 sm:mt-0 sm:col-span-1">
          ...
          <dd className="mt-1 text-sm text-gray-900">{lesson.level}</dd>
        </div>
        <div className="mt-4 sm:mt-0 sm:col-span-1">
          <dt className="text-sm font-medium text-gray-500">Order</dt>
          <dd className="mt-1 text-sm text-gray-900">{lesson.order}</dd>
        </div>
      </div>
      ...
      {activeTab === 'vocabulary' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-medium text-gray-900">Vocabulary Words</h3>
            <button
      ...
      <ConfirmDialog
        ...
        title={`Delete ${deleteType === 'vocabulary' ? 'Vocabulary' : 'Question'}`}
